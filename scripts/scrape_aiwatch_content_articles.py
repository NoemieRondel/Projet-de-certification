...
def fetch_aiwatch_content(url):
    """Récupérer et parser le contenu d'un article AI Watch."""
    try:
        headers = {
        ...
        if response.status_code == 429:
            ...
            time.sleep(60)
            return fetch_aiwatch_content(url)
...
def main():
    """Script principal pour récupérer et stocker le contenu des articles AI Watch."""
    try:
        connection = connect_db()
        cursor = connection.cursor(dictionary=True)

        cursor.execute("SELECT id, link FROM articles WHERE source = 'AI Watch European Commission'")
        ...
        for article in articles:
            ...
            content = fetch_aiwatch_content(url)
            if not content:
